...
public class Diary implements Serializable {
    ...
    public void unlockDiary(String password) {
        if (this.password.equals(password)) locked = false;
    }

    public void createEntry(String title, String body) {
        if (locked) throw new IllegalStateException("Diary is locked, Unlock it first.");
        ...
        if (findEntryByTitle(title) != null) throw new IllegalArgumentException("An entry with that title already exists.");
    ...
    private void loadDiary() {
        ...
        try (ObjectInputStream load = new ObjectInputStream(new FileInputStream(diaryFile))) {
            Diary loadedDiary = (Diary) load.readObject();
    ...
    private void saveDiary() {
        try (ObjectOutputStream save = new ObjectOutputStream(new FileOutputStream(diaryFile))) {
